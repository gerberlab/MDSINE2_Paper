...
if __name__ == '__main__':
    ...
    # ---------------------
    print('\n\nExtract v4 region')
    fname_v4_aligned_refpkg = os.path.join(temp_folder, 'aligned_reference_sequences_v4_region.fa')
    ...
    hmm_model = os.path.join(temp_folder, 'aligned_reference_sequences_v4_region.hmm')
    ...
    align_output_v4 = os.path.join(output_folder, 'placed_sequences_on_v4_region.sto')
    ...
    # ---------------------------------------------
    print('\n\nPlace aligned sequences on the reference tree')
    pplacer_output = os.path.join(output_folder, 'placement.jplace')
    ...
    xml_output = os.path.join(temp_folder, 'xml_tree.xml')
    ...
    species_output = os.path.join(output_folder, 'taxonomy.xml')
    ...
    # ---------------------
    print('\n\nMaking newick tree')
    newick_output = os.path.join( output_folder, 'newick_tree_full_taxid.nhx')
    Phylo.convert(xml_output, 'phyloxml', newick_output, 'newick')

    newick_output = os.path.join( output_folder, 'newick_tree_full_speciesName.nhx')
    xml_tree = xml_output.replace('.xml', '_speciesName.xml')
    Phylo.convert(xml_tree, 'phyloxml', newick_output, 'newick')
    ...
    tree.write(format=1, outfile=os.path.join(output_folder, 'newick_tree_query_reads.nhx'))
