...
timeseries_id=$5
inference_dir=$6
full_timeseries_inference_dir=$7
...
require_variable "full_timeseries_inference_dir" "${full_timeseries_inference_dir}"
...
if [[ "$timeseries_id" == "all" ]]; then
  echo "[* filter_like_densest_timeseries.sh] Filtering timeseries \"all\" using default filtering criteria."
  bash semisynthetic2/inference/mdsine2/filter.sh ${traj_repl} ${n_perts} ${data_repl} ${n_mice} ${timeseries_id} ${inference_dir}
else
  echo "[* filter_like_densest_timeseries.sh] Filter for \"${timeseries_id}\" using taxaset from \"all\" (${full_timeseries_inference_dir})."
  ...
  python semisynthetic2/inference/mdsine2/helpers/filter_like_other.py \
    ...
    --like-other "${full_timeseries_inference_dir}/synthetic_filtered.pkl" \
    -o "${inference_dir}/synthetic_filtered.pkl"

  echo "[* filter_like_densest_timeseries.sh] Applying dataset filter to replicates."
  python semisynthetic2/inference/mdsine2/helpers/filter_like_other.py \
    -i "${this_pickle_dir}/synthetic_replicate.pkl" \
    --like-other "${full_timeseries_inference_dir}/synthetic_filtered.pkl" \
