...
inference_out_dir=$OUTPUT_DIR/mdsine2-modules-mhlearn/${excluded_subj}
mkdir -p ${inference_out_dir}

echo "[*] Running cross-validation run for MDSINE2: SI/Growths learned + small-scale SICS prior (Excluding subject: ${excluded_subj})"

# 870 = N * (N-1), where N=30
# interaction-str-var-scale chosen so that mean variance is 1e-10, using DOF of 870.
python cross_validation/helpers/mdsine2_loo_mhlearn.py \
		--input $DATASET_PKL \
