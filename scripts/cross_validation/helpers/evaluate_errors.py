...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--glv_ra_elastic', type=str, required=True)
    parser.add_argument('--glv_ra_ridge', type=str, required=True)
    parser.add_argument('--glv_ridge', type=str, required=True)
    parser.add_argument('--lra_elastic', type=str, required=True)
...
class HoldoutData:
    def __init__(self, subject: md2.Subject):
    ...
    def evaluate(self, pred: np.ndarray, lb: float = 1e5) -> float:
...
def cached_forward_simulation(fwsim_fn: Callable):
...
def forward_sim_mdsine2(data_path: Path, heldout: HoldoutData, sim_dt: float, sim_max: float) -> np.ndarray:
    print(f"Evaluating forward simulation using mdsine2 MCMC samples ({data_path})")
...
@cached_forward_simulation
def forward_sim_clv(data_path: Path, heldout: HoldoutData) -> np.ndarray:
    from compositional_lotka_volterra import CompositionalLotkaVolterra
    with open(data_path, 'rb') as pkl:
        clv: CompositionalLotkaVolterra = pickle.load(pkl)
...
class HeldoutInferences:
    ...
    def clv_elastic_fwsim(self, heldout: HoldoutData) -> np.ndarray:
        return forward_sim_clv(data_path=self.clv_elastic, subject=heldout)


def retrieve_grouped_results(directories: HeldoutInferences) -> Iterator[Tuple[str, HeldoutInferences]]:
    """
    ...
    for subject_idx, subject_id in enumerate(SUBJECT_IDS):
        yield subject_id, HeldoutInferences(
...
def main():
    ...
    directories = HeldoutInferences(
        ...
        glv_ra_elastic=Path(args.glv_ra_elastic),
        glv_ra_ridge=Path(args.glv_ra_ridge),
        glv_ridge=Path(args.glv_ridge),
        lra_elastic=Path(args.lra_elastic),
    )

    complete_study = md2.Study.load(args.study)
    for heldout_subj_id, inferences in retrieve_grouped_results(directories):
        heldout_data = HoldoutData(complete_study[heldout_subj_id])
        heldout_data.evaluate(inferences.mdsine2_fwsim(heldout_data, args.sim_dt, args.sim_max))
        heldout_data.evaluate(inferences.clv_elastic_fwsim(heldout_data))
        # truth.evaluate(inferences.glv_elastic_fwsim)
        # truth.evaluate(inferences.glv_ra_elastic_fwsim)
        # truth.evaluate(inferences.glv_ra_ridge_fwsim)
        # truth.evaluate(inferences.glv_ridge_fwsim)
        # truth.evaluate(inferences.lra_elastic_fwsim)
