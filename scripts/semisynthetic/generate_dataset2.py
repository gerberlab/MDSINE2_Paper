...
def create_synthetic_replicates(
    ...
    trajectory_set: md2.posterior.TrajectorySet = source_mcmc.graph[STRNAMES.LATENT_TRAJECTORY]
    traj_posterior_samples = trajectory_set.value[src_subj.index].get_trace_from_disk(section='posterior')
    true_traj = np.median(traj_posterior_samples, axis=0)  # (taxa x timepoint)
    ...
    for replicate_subj, subj_t in zip(replicate_study, source_subj_timepoints):
        ...
        for repl_t in replicate_times:
            # Make the reads
            alpha = dirichlet_alpha * rel_mass
            replicate_subj.reads[repl_t] = dirichlet_multinomial(alpha, num_reads, rng=rng)
...
def sample_data_from_posterior_trajectory(
    ...
    for subj_idx, subj in enumerate(study):
        ...
        if n_union_timepoints != len(subj.times):
            print(
                "Subject `{}`: Matrix's n_timepoints dimension ({}) doesn't match measurement timepoints ({}). Reverse timepoint lookup is required.".format(
            ...
            """
            _union_indices = {t: i for i, t in enumerate(
                mcmc.graph.data.times[subj_idx])}  # indices of the LARGE timepoint array (with fake timepoints)
            _t_indices = [_union_indices[t] for t in
                          subj.times]  # The ORIGINAL timepoints from the subject's raw measurements
            traj_posterior_samples = traj_posterior_samples[:, :, _t_indices]  # the submatrix containing only the target measurement timepoints.

        # sample reads from median latent state trajectory
        reads_per_subject[subj.name] = sample_reads(
            read_depth, np.median(traj_posterior_samples, axis=0), dirichlet_alpha, rng=rng
        )
...
def main(
    ...
    src_study = md2.Study.load(str(study_path))
    src_mcmc = md2.BaseMCMC.load(str(fixed_module_pkl_path))
    synth_study = create_synthetic_dataset(
        source_study=src_study,
        fixed_cluster_mcmc=src_mcmc,
