...
class roc():
    def __init__(self, p_vals, labels):
        """
           (np.array) p_vals : vector of p-valuues
           (np.array) labels : vector of binary-labels(0 or 1)
        """
        self.p = p_vals
        self.y = labels
        thresholds = np.linspace(0, 1, 1000)
        self.fpr, self.tpr = self.compute_stats(thresholds)
    ...
    def plot_ROC(self, savename):
        ...
        axes.title('ROC Curve')

        axes.plot(np.linspace(0, 1, 100), np.linspace(0, 1, 100), "--",
            color="black")
        axes.plot(self.fpr, self.tpr)

    def compute_stats(self, cutoffs):
        """computes the True Positive Rate and False Positive Rates"""
        tpr_all = []
        fpr_all = []
        for c in cutoffs:
            y_pred = np.where(self.p_vals <= c, 1, 0)
            tp = np.sum(np.logical_and(y_pred==1, self.y==1))
            fp = np.sum(np.logical_and(y_pred==1, self.y==0))
            tn = np.sum(np.logical_and(y_pred==0, self.y==0))
            fn = np.sum(np.logical_and(y_pred==0, self.y==1))

            tpr_all.append(tp / (tp + fn))
            fpr_all.append(fp / (fp + tn))
        return np.array(fpr_all, tpr_all)
